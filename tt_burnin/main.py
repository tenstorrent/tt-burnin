...
def start_burnin_gs(device):
    raise NotImplementedError("Have not implemented burnin for gs")
...
def start_burnin_wh(device, keep_trisc_under_reset: bool = False):
    ...
    device.noc_broadcast32(0, 0xffb121b0, BRISC_SOFT_RESET | TRISC_SOFT_RESETS | NCRISC_SOFT_RESET)
    ...
    to_deassert = load_ttx_file(device, TtxFile("/mnt/motor/syseng/ttx-bank/wh_B0/pv_workloads/build_pv_ssmodes_sync_v3_fp32acc_dcache_off/single-core-matrix-inf-loop-20act.80wght-lf8.ttx"), {
        CoreId(0, 0): device.get_tensix_locations()
    })

    if keep_trisc_under_reset:
        soft_reset_value = NCRISC_SOFT_RESET | TRISC_SOFT_RESETS | STAGGERED_START_ENABLE
    else:
...
def main():
    ...
    try:
        ...
        input("Press Enter to STOP TT-Burnin on all boards (Please close all other processes running on the boards FIRST)")
    finally:
        for device in devices:
            if isinstance(device, GsChip):
                continue
